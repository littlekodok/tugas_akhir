...
class PajakHotelController extends Controller
{
    ...
    {
        return view('admin.pajak-hotel.index');
    }
    public function dataHotelBelumValidasiJson()
    {
        $query = ObjekPajak::where('id_jenis_pajak' ,'=','1')->where('validasi','=','Proses');

        return DataTables::of($query)
                        ->addIndexColumn()
                        ->addColumn('aksi', function ($data){
                            return '<a href="'. route('admin.pajak-hotel.validasi',$data->id) .'" class="btn btn-primary btn-xs"><i class="fas fa-pencil-alt"></i></a>';
                        })
    ...
    public function dataHotelTerValidasiJson()
    {
        // $query = ObjekPajak::where('id_jenis_pajak' ,'=','1')->where('validasi','=','Sukses');
        $query = ObjekPajak::where('id_jenis_pajak' ,'=','1')->where(
            function($query) {
              return $query
        ...
        return DataTables::of($query)
                        ->addIndexColumn()
                        ->addColumn('aksi', function ($data){
                            return '<button type="button" value="'.$data->id.'" class="btn btn-primary modal-hotel btn-sm"><i class="fa-solid fa-magnifying-glass"></i></button>';
                        })
    ...
    public function validasi(ObjekPajak $objekpajak)
    {
        ...
        return view('admin.pajak-hotel.validasi',compact('objekpajak'));
    }
    public function validasi_post(ObjekPajak $objekpajak,Request $request)
    {
        ...
        return redirect()->route('admin.data-objek-pajak.pajak-hotel')->withSuccess('Pajak Hotel Berhasil Divalidasi');
    }
